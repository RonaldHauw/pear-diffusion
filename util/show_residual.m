...
function show_residual( sol, name, elements3, coordinates, varargin )
    ...
    %clim = [0, 10] ;
    clim = [0, max(abs(sol(1:M)))] ;
    subplot(1, 2, 1)
    box on
    hold on
    trisurf(elements3, coordinates(:,1), coordinates(:,2), full( abs(sol(1:M)) )','facecolor','interp', 'linestyle', 'none')
    hold off
    view(0,90);
    grid off
    ...
    clim = [0, max(abs(sol(M+1:end)))] ;
    
    subplot(1, 2, 2)
    box on
    hold on
    trisurf(elements3, coordinates(:,1), coordinates(:,2), full( abs(sol(M+1:end)) )','facecolor','interp', 'linestyle', 'none')
    hold off
    view(0,90);
    grid off
    xlim( [min(coordinates(:, 1)), max(coordinates(:, 1))] )
